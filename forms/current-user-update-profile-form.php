<?php if(!empty($current_user_data)): ?>
    <form action="/supercar/dashboard/profile" method="POST" class="w-auto flex flex-col gap-y-2">
        <div class="fullname grid grid-cols-1 gap-3">
            <label class="input input-bordered flex items-center gap-2">
                ...
                <input type="text" placeholder="Prenom" name="firstname" required
                       ...
                       value="<?php echo $current_user_data["prenom"];?>"
                />
            </label>
            <label class="input input-bordered flex items-center gap-2">
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-6 opacity-70 text-secondary">
                ...
                <input type="text" placeholder="Nom" name="lastname" required title="minimum 2 caractères"
                       pattern="^(?!\s*$).{2,}$" class="grow"
                       value="<?php echo $current_user_data["nom"]; ?>"
                />
            </label>
        </div>
        <div class="phone-email grid grid-cols-1 gap-3">
            <label class="input input-bordered flex items-center gap-2">
                ...
                <input type="text" placeholder="Téléphone" name="phone" required pattern="^\+\d{1,4}(\s?\d{7,12})+$"
                       ...
                       value="<?php echo $current_user_data["telephone"]; ?>"
                />
            </label>
            <label class="input input-bordered flex items-center gap-2">
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-6 opacity-70 text-secondary">
                ...
                <input type="email" class="grow" placeholder="Email" required name="email"
                       value="<?php echo $current_user_data["email"]; ?>"
                />
            </label>
        </div>
        <div class="phone-email grid grid-cols-1 gap-3">
            <label class="input input-bordered flex items-center gap-2">
                ...
                <input type="text" placeholder="Adresse" name="address" required
                       title="minimum 5 caractères" pattern="^(?!\s*$).{2,}$" class="grow"
                       value="<?php echo $current_user_data["adresse"]; ?>"
                />
            </label>
        </div>
