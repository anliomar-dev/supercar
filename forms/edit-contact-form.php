<?php if(!empty($current_contact)) :?>
    <div class="edit-form section w-full flex justify-center bg-base-100 py-10 rounded-lg">
        <form action="/supercar/admin/contacts/update" method="POST" class="w-1/2 flex flex-col gap-y-2 bg-base-100 p-5 rounded-lg shadow-md">
            <div class="login-form-title flex justify-center">
                <h2 class="text-xl font-bold font-comic py-4">Modifier le contact</h2>
            </div>
            <input type="hidden" name="action" value="edit">
			<input type="hidden" name="id_contact" value="<?php echo $current_contact["id_contact"]; ?>">
            <div class="fields flex flex-col gap-y-3">
                ...
                <div class="fullname-fields grid grid-cols-1 md:grid-cols-2 gap-3">
                    <label class="input input-bordered flex items-center gap-2">
                        ...
                        <input type="text" placeholder="Prenom" name="firstname" value="<?php echo $current_contact["prenom"]; ?>"
							   required title="minimum 2 caractères" pattern="^(?!\s*$).{2,}$" class="grow"
						/>
                    </label>
                    <label class="input input-bordered flex items-center gap-2">
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-6 opacity-70 text-secondary">
                        ...
                        <input type="text" placeholder="Nom" name="lastname" required title="minimum 2 caractères"
							   pattern="^(?!\s*$).{2,}$" class="grow" value="<?php echo $current_contact["nom"]; ?>" />
                    </label>
                </div>
                ...
                <div class="phone-email-fields grid grid-cols-1 md:grid-cols-2 gap-3">
                    <label class="input input-bordered flex items-center gap-2">
                        ...
                        <input type="text" placeholder="Téléphone" name="phone" required pattern="^\+\d{1,4}(\s?\d{7,12})+$"
                               title="Le numéro doit être au format international. exp: +230 5429 7857" class="grow" value="<?php echo $current_contact["telephone"]; ?>" />
                    </label>
                    <label class="input input-bordered flex items-center gap-2">
                        ...
                        <input type="email" class="grow" placeholder="Email" required name="email" value="<?php echo $current_contact["email"]; ?>" />
                    </label>
                </div>
            </div>
