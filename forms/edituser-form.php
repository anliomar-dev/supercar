		...
		<div class="fields flex flex-col gap-y-3">
			...
			<div class="fullname-fields grid grid-cols-1 md:grid-cols-2 gap-3">
				<label class="input input-bordered flex items-center gap-2">
					...
					<input type="text" placeholder="Prenom" name="firstname" value="<?php echo $curent_user["prenom"]; ?>" required title="minimum 2 caractères" pattern="^(?!\s*$).{2,}$" class="grow" />
				</label>
				<label class="input input-bordered flex items-center gap-2">
					<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-6 opacity-70 text-secondary">
						<path stroke-linecap="round" stroke-linejoin="round" d="M15.75 6a3.75 3.75 0 1 1-7.5 0 3.75 3.75 0 0 1 7.5 0ZM4.501 20.118a7.5 7.5 0 0 1 14.998 0A17.933 17.933 0 0 1 12 21.75c-2.676 0-5.216-.584-7.499-1.632Z" />
					</svg>
					<input type="text" placeholder="Nom" name="lastname" value="<?php echo $curent_user["nom"]; ?>" required title="minimum 2 caractères" pattern="^(?!\s*$).{2,}$" class="grow" />
				</label>
			</div>
			...
			<div class="adresse">
				<label class="input input-bordered flex items-center gap-2">
					...
					<input type="text" placeholder="Adresse" name="address" value="<?php echo $curent_user["adresse"]; ?>" required title="minimum 5 caractères" pattern="^(?!\s*$).{2,}$" class="grow" />
				</label>
			</div>
			...
			<div class="phone-email-fields grid grid-cols-1 md:grid-cols-2 gap-3">
				<label class="input input-bordered flex items-center gap-2">
					...
					<input type="text" placeholder="Téléphone" name="phone" value="<?php echo $curent_user["telephone"]; ?>" required pattern="^\+\d{1,4}(\s?\d{7,12})+$"
				...
				<label class="input input-bordered flex items-center gap-2">
					...
					<input type="email" class="grow" placeholder="Email" required name="email" value="<?php echo $curent_user["email"]; ?>" />
				</label>
			</div>
