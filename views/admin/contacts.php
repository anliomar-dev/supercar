		...
		<div class="drawer-content flex flex-col items-center px-3 lg:px-8 py-4">
			...
			<div class="flex justify-between items-center w-full mt-5 py-3">
				<?php if(isset($_GET["contact"])): ?>
					<div class="join">
						<a href="/supercar/admin/contacts" class="btn btn-accent btn-outline rounded-lg">
				...
				<?php else: ?>
					<div class="join">
						<?php if(str_ends_with($_SERVER["REQUEST_URI"], "create")): ?>
							<a href="/supercar/admin/contacts" class="btn join-item">Contacts</a>
							<a href="/supercar/admin/contacts/create" class="btn btn-primary join-item">Ajouter</a>
						<?php else:?>
							<a href="/supercar/admin/contacts" class="btn btn-primary join-item">Contacts</a>
							<a href="/supercar/admin/contacts/create" class="btn join-item">Ajouter</a>
						<?php endif; ?>
					</div>
				<?php endif; ?>
			</div>
			<?php if(isset($paginated_contacts["data"])): ?>
				<form class="overflow-x-auto w-full bg-base-100 rounded-lg shadow-sm section list-form">
					<table class="table">
						...
						<tbody>
							<?php $i = 1; ?>
							<?php foreach($paginated_contacts["data"] as $contact): ?>
								<tr>
									<td>
										<div class="flex items-center gap-3">
											<div class="avatar avatar-placeholder">
												<div class="mask mask-squircle bg-neutral text-neutral-content w-10 ">
													<span>
														<?php echo strtoupper($contact["nom"][0]) . strtoupper($contact["prenom"][0]); ?>
													</span>
												</div>
											</div>
											<div>
												<div class="text-sm"><?php echo $contact["nom"]; ?></div>
												<div class=""><?php echo $contact["prenom"]; ?></div>
											</div>
										</div>
									</td>
									<td>
									...
									<th>
										<a class="btn btn-ghost btn-xs" href="/supercar/admin/contacts?contact=<?php echo $contact["id_contact"]; ?>">
											<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-6">
												<path stroke-linecap="round" stroke-linejoin="round" d="m16.862 4.487 1.687-1.688a1.875 1.875 0 1 1 2.652 2.652L10.582 16.07a4.5 4.5 0 0 1-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 0 1 1.13-1.897l8.932-8.931Zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0 1 15.75 21H5.25A2.25 2.25 0 0 1 3 18.75V8.25A2.25 2.25 0 0 1 5.25 6H10" />
											</svg>
										</a>
										<a class="btn btn-ghost btn-xs" href="/supercar/admin/contacts/delete?contact=<?php echo $contact["id_contact"]; ?>">
							...
							<tr>
								<td colspan="4">
									<div class="flex justify-end p-4">
										<!-- Pagination -->
										<?php if($paginated_contacts["total_page"] > 0):?>
											<div class="join">
												<!--prev button-->
												...
												<a class="join-item btn btn-primary text-white">
													<?php echo $paginated_contacts["current_page"]; ?>/<?php echo $paginated_contacts["total_page"]; ?>
												</a>
			...
			<?php elseif(isset($current_contact)): ?>
